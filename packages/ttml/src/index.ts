...
export * from "./parser";
export type {
	AmllLyricLine as LyricLine,
	AmllLyricWord as LyricWord,
	AmllMetadata as TTMLMetadata,
	LyricWordBase,
	TTMLLyric,
} from "./types";
...
/**
 * 将 {@link TTMLLyric} 对象序列化为 TTML 格式的 XML 字符串的便捷方法
 *
 * 若需要自定义生成选项，建议直接使用 {@link TTMLParser} 类
 * @warn 默认使用全局的 {@link document.implementation} 和 {@link xmlSerializer}，若为 Nodejs 环境，
 * 必须使用 {@link TTMLGenerator} 类注入 `domImplementation` 和 `xmlSerializer`，例如 `@xmldom/xmldom`
 * @param ttmlLyric 包含歌词行列表和元数据的 {@link TTMLLyric} 对象
 * @returns 序列化后的 TTML XML 字符串
 * @throws 如果没有全局的 `DOMImplementation` 和 `XMLSerializer`，抛出错误
 */
export function exportTTML(ttmlLyric: TTMLLyric): string {
