...
| [LrcLib 精确匹配](lrclib#精确匹配)           | `GET`     | `/v1/lrclib/get`      | 否       | 兼容 LrcLib 协议的精确匹配与歌词获取接口    |
...
1. **原生 API 包装响应 `ApiResponse<T>`**（适用于 [/native](native) 接口）：
   统一采用 `ApiResponse<T>` 结构包装。正常响应时 `status` 固定为 `200`，`data` 泛型包含具体的业务数据主体；请求失败时返回 `ErrorResponse` 模型
...
## 原生 API 包装模型 `ApiResponse<T>`

原生接口在请求成功时，均统一使用 `ApiResponse<T>` 泛型接口包装数据
...
- `status`: 响应状态码，固定为 `200`
- `data`: 泛型业务数据载荷。例如在 [搜索接口](native#搜索歌词) 中 `T` 为 `{ items: SongItem[] }`；在 [获取歌词接口](native#获取歌词) 中 `T` 为 [SongItem](#歌曲元数据与歌词模型-songitem)
