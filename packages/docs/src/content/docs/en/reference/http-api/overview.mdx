...
| [LrcLib Fuzzy Match](lrclib#fuzzy-match)       | `GET`       | `/v1/lrclib/get`      | No            | LrcLib protocol compatible fuzzy match and lyric retrieval endpoint |
...
- `SuccessResponse<T>`: Successful response model, `data` contains generic business data payload. E.g., in the [Search Endpoint](native#search-lyrics), `T` is `{ items: SongItem[]; pagination: PaginationInfo }`; in the [Get Lyric Endpoint](native#get-lyric), `T` is [SongItem](#song-metadata--lyric-model-songitem)

---

## Pagination Metadata Model `PaginationInfo`

Pagination metadata returned by the native search endpoint, including current page, items per page, and total count.
...
export interface PaginationInfo {
  ...
  pageSize: number;
  /** Total number of matched items for the query, unaffected by page / pageSize */
  total: number;
  /** Total number of pages; 0 when total is 0 */
  totalPages: number;
  /** Whether more results exist after the current page, equivalent to page < totalPages */
  hasMore: boolean;
}
```
