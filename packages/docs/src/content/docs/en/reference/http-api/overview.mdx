...
1. **Native API Wrapped Response `ApiResponse<T>`** (used for [/native](native) endpoints):
   Wrapped uniformly in an `ApiResponse<T>` structure. Successful responses fix `status` to `200`, with `data` containing business data. Failed requests return an `ErrorResponse` model.
...
## Native API Wrapper Model `ApiResponse<T>`

Native endpoints use the `ApiResponse<T>` generic interface to wrap data on successful requests:
...
- `status`: Response status code, fixed to `200`
- `data`: Generic business data payload. E.g., in the [Search Endpoint](native#search-lyrics), `T` is `{ items: SongItem[] }`; in the [Get Lyric Endpoint](native#get-lyric), `T` is [SongItem](#song-metadata--lyric-model-songitem)
...
Error response model returned on request failures:
