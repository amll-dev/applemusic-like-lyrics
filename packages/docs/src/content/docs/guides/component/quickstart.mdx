...
// update 需要逐帧调用；播放时同步当前进度给 setCurrentTime
let lastFrameTime = -1;
...
audio.addEventListener("seeked", () => {
	// 对于较大的时间跳变，需为 setCurrentTime 提供第二个参数 isSeek 标志
	// 此时组件会强制对齐位置，对所有歌词整体执行快速移动效果
...
其中的 [`setLyricLines`](/reference/core/classlyricplayerbase#setlyriclines) 方法接受 [`LyricLine`](/reference/core/interfacelyricline) 类型的对象。在传入之后不应再修改这一对象的属性，否则可能导致问题。`parseTTML` 返回的是包含元数据的对象，真正传给组件的是其中的 `.lines`。

[`setCurrentTime`](/reference/core/classlyricplayerbase#setcurrenttime) 方法接受当前播放进度（整数，单位毫秒）作为参数；[`update`](/reference/core/classlyricplayerbase#update) 方法接受距离上一帧的时间差（单位毫秒）作为参数。使用原生方式时需要逐帧调用 `update`，并在播放时同步调用 `setCurrentTime`；跳转后可以传入第二个参数 `true` 强制对齐歌词位置。
...
React 绑定包拥有具名导出 [`LyricPlayer`](/reference/react/variablelyricplayer) 作为核心组件。下面是一段示例，假设 TTML 歌词文件在 `/lyrics/song.ttml` 上提供。
	...
	useEffect(() => {
		...
		const onFrame = () => {
			...
			if (audio && !audio.paused) {
				// React 绑定会在 currentTime 变化时同步到底层歌词组件
...
Vue 绑定包拥有具名导出 [`LyricPlayer`](/reference/vue/classlyricplayer) 作为核心组件。歌词对象、播放进度等均以组件的响应式属性传入。下面是一段示例，假设 TTML 歌词文件在 `/lyrics/song.ttml` 上提供。
...
function syncCurrentTime() {
	if (audioEl.value) {
		// Vue 绑定会在 currentTime 变化时同步到底层歌词组件
