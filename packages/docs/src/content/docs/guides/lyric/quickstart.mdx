...
| Lyricify Syllable | `.lys`           | [`parseLys`](/reference/lyric/functionparselys)     | [`stringifyLys`](/reference/lyrilys)                        |
...
TTML 包提供两个类：用于解析的 [`TTMLParser`](/reference/ttml/classttmlparser) 和用于生成的 [`TTMLGenerator`](http://localhost:4321/reference/ttml/classttmlgenerator)。特别地，二者还提供了静态的便捷方法，本质是立即新建实例然后操作。
...
直接引入 [`parseTTML`](http://localhost:4321/reference/ttml/functionparsettml), [`exportTTML`](/reference/ttml/functionexportttml) 函数。
